...
void processWebSocketMessage(String str, int dataVar){
      ...
      else if (str == "SOFF"){offdis = dataVar; if (tower && (effect_function == *snow_flakes || effect_function == *snow_flakes_2 || effect_function == *meteor)){offdisC = offdis; if (saveToEEPROM){EEPROM.put(offsetof(storeInEEPROM, offdisC), offdisC);  EEPROM.commit();};};}
